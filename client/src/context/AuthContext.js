...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password, loginAs) => {
    const res = await api.post('/api/api/auth/login', { email, password, loginAs });
    if (res.data.success) {
  ...
  const registerFarmer = async (data) => {
    const res = await api.post('/api/api/auth/farmer/register', data);
    if (res.data.success) {
      return res.data;
    }
  ...
  const registerExpert = async (data) => {
    const res = await api.post('/api/api/auth/expert/register', data);
    if (res.data.success) {
      return res.data;
    }
    throw new Error('Registration failed');
  ...
  const logout = async () => {
    ...
    if (refreshToken) {
      try {
        await api.post('/api/api/auth/logout', { refreshToken });
      } catch {
